...
class Conversions:
    ...
    @commands.command(pass_context=True)
    async def fiat(self, ctx, coin, ammount:float=1.0, currency="USD"):
        ...
        if coin_data is None:
            ...
            return      
        price = (1.0/float(coin_data["price_usd"])) * ammount
        ...
        msg = "{0:.2f} {2} is {1:.8f} {3}".format(ammount, price, currency.upper(), coin_data["symbol"])
    ...
    async def crypto_embed(self, ctx, coin, ammount=1.0, currency="USD", full=True):
        ...
        if full:
            ...
            embed.add_field(name="Rank", value=coin_data["rank"], inline=True)
