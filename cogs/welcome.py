...
default_settings = {"GREETING": default_greeting, "LEAVING":"", "LEAVE ON": False ,"ON": False, "CHANNEL": None, "WHISPER" : False}
...
class Welcome:
    ...
    async def welcomeset(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            ...
            msg += "LEAVING: {}\n".format(self.settings[server.id]["LEAVING"])
            msg += "CHANNEL: #{}\n".format(self.get_welcome_channel(server)) 
            msg += "ON: {}\n".format(self.settings[server.id]["ON"]) 
            msg += "LEAVE ON: {}\n".format(self.settings[server.id]["LEAVE ON"]) 
    ...
    async def greeting(self, ctx, *, format_msg):
        ...
        await self.send_testing_msg(ctx, "GREETING")
    ...
    async def leaving(self, ctx, *, format_msg):
        ...
        """
        server = ctx.message.server
        self.settings[server.id]["LEAVING"] = format_msg
        fileIO("data/welcome/settings.json","save",self.settings)
        await self.bot.say("Welcome message set for the server.")
        await self.send_testing_msg(ctx, "LEAVING")
    ...
    async def toggleleave(self, ctx):
        """Turns on/off saying goodbye to users on the server"""
        server = ctx.message.server
        self.settings[server.id]["LEAVE ON"] = not self.settings[server.id]["LEAVE ON"]
        if self.settings[server.id]["LEAVE ON"]:
            await self.bot.say("I will now say goodbye to users from the server.")
            await self.send_testing_msg(ctx, "LEAVING")
        else:
            await self.bot.say("I will no longer welcome new users.")
        fileIO("data/welcome/settings.json", "save", self.settings)
    ...
    async def member_leave(self, member):
        ...
        if server.id not in self.settings:
            ...
            self.settings[server.id]["CHANNEL"] = server.default_channel.id
            fileIO("data/welcome/settings.json","save",self.settings)
        if not self.settings[server.id]["LEAVE ON"]:
            return
        if server == None:
            print("Server is None. Private Message or some new fangled Discord thing?.. Anyways there be an error, the user was {}".format(member.name))
            return
        ...
        if channel is None:
            ...
            return
        if self.settings[server.id]["WHISPER"]:
            await self.bot.send_message(member, self.settings[server.id]["LEAVING"].format(member, server))
        if self.settings[server.id]["WHISPER"] != True and self.speak_permissions(server):
            await self.bot.send_message(channel, self.settings[server.id]["LEAVING"].format(member, server))
        else:
            print("Permissions Error. User that joined: {0.name}".format(member))
            print("Bot doesn't have permissions to send messages to {0.name}'s #{1.name} channel".format(server,channel))
    ...
    async def send_testing_msg(self, ctx, joinleave):
        server = ctx.message.server
        channel = self.get_welcome_channel(server)
        if channel is None:
        ...
        if self.speak_permissions(server):
            if self.settings[server.id]["WHISPER"]:
                await self.bot.send_message(ctx.message.author, self.settings[server.id][joinleave].format(ctx.message.author,server))
            if self.settings[server.id]["WHISPER"] != True:
                await self.bot.send_message(channel, self.settings[server.id][joinleave].format(ctx.message.author,server))
        else: 
