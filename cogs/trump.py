...
class Trump:
    ...
    async def trump(self, ctx, *, message):
        ...
        lastCorners = None
        if len(text) == 1:
            textImage = await self.generateText(text[0])
        elif len(text) == 2:
            textImage = await self.generateText(text[0], text[1])
        elif len(text) == 3:
            textImage = await self.generateText(text[0], text[1], text[2])
    ...
    async def generateText(self, text, text2="IS NOW", text3="ILLEGAL"):
        ...
        draw.text((12, 70), text2, font=self.impact, fill=txtColor)
        draw.text((10, 130), text3, font=self.impact, fill=txtColor)
