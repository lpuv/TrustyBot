import praw, operator, string, csv, json, numpy as np
...
class BotFinder:
    """Reddit bot detection by /u/perrycohen"""

    def __init__(self, bot):
        self.bot = bot

    @commands.command(pass_context=True)
    async def isbot(self, ctx, *, username):
        algorithm = Classifier(0.5)
        url = "https://reddit.com/u/{}".format(username)
        try:
            is_bot, conf = algorithm.is_a_bot(username)
        except:
            is_bot, conf = None, None
        if is_bot:
            await self.bot.say("I am {}%  confident {} is a bot.\n{}".format(conf*100, username, url))
        elif is_bot is None:
            await self.bot.say("I don't have enough data to determine if {} is a bot.\n{}".format(username, url))
        else:
            await self.bot.say("I am {}%  confident {} is not a bot.\n{}".format(100-conf*100, username, url))


np.random.seed(1)
r = praw.Reddit('bot1', user_agent='CorrectTheRhubarb')
